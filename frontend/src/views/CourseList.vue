    ...
    <el-card class="!rounded-2xl !border-none shadow-premium overflow-hidden !bg-white">
      ...
      <div class="table-container bg-white rounded-xl overflow-hidden border border-gray-50">
        ...
        >
          ...
          <el-table-column prop="teacher" label="授课教师" min-width="140" />
    ...
    >
      ...
      >
        ...
        <el-form-item label="学分" prop="credit">
          <el-input-number v-model="formData.credit" :min="0" :max="10" :step="0.5" class="!w-full" />
        </el-form-item>
      ...
      <template #footer>
        <div class="flex justify-end space-x-3 p-4">
          ...
          <el-button type="primary" @click="handleSubmit" class="!rounded-xl">确定</el-button>
        </div>
      </template>
    </el-dialog>
  </div>
...
import { getCoursePage, saveCourse, updateCourse, deleteCourse } from '../api/course'
...
const handleDelete = (row) => {
  ElMessageBox.confirm(
    `此操作将永久移除课程 [${row.courseName}]，是否继续?`,
    '危险操作警告',
    {
...
const handleSubmit = async () => {
  ...
  await formRef.value.validate(async (valid) => {
    if (valid) {
      try {
        const res = isEdit.value ? await updateCourse(formData.value) : await saveCourse(formData.value)
        if (res.code === 200) {
          ElMessage.success({
            message: isEdit.value ? '课程信息已更新' : '课程已新增',
            plain: true
          })
